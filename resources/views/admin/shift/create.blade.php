    ...
    <body>
            ...
            <form action="/admin/dashboard" method="POST" enctype="multipart/form-data">
                ...
                <div class = "text-base text-center mt-12">
                    ...
                    <input class = "mt-7 rounded-lg border border-gray-500" type="date" name="shift[date]" value="{{ old('shift.date') }}" />
                    <p class="text-red-500">{{ $errors->first('shift.date') }}</p>
                </div>
                <div class="text-center mt-20">
                    <input type="file" name="shift[image]" value="{{ old('shift.image') }}"/>
                    <p class="text-red-500">{{ $errors->first('shift.image') }}</p>
                </div>
